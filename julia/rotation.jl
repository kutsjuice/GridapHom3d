...
GridapPETSc.with(args=options) do
    ...
    test_space_V = TestFESpace(model, reffe, conformity=:H1, dirichlet_tags = ["n1_first", "n1_second"])

    g(x) = VectorValue(0.0,  -x[1]*x[3], x[1]*x[2])

    trial_space_U = TrialFESpace(test_space_V, [g, g]) # Создаем пространство тестовых функций.
    ...
    # rhs_vector
    ##
    res_file = "rotations_new" |> results
