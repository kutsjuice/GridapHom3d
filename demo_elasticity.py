...
def demo_elasticity():
    ...
    s_m_c = {l2b([1, 0]): {l2b([1, 1]): 1.0}}
    print("MPC DICCT IS AS FOLLOW::  ", s_m_c)
    mpc = MultiPointConstraint(V)
    mpc.create_general_constraint(s_m_c, 1, 1)
    mpc.create_general_constraint(s_m_c, 0, 0)
    mpc.finalize()
    ...
    u_h = problem.solve()
    u_h.name = "u_mpc"
    outdir = Path("demo_elasticity")
    outdir.mkdir(exist_ok=True, parents=True)

    with VTKFile(mesh.comm, outdir / "demo_elasticity_constrained", "w") as outfile:
    ...
    with VTKFile(mesh.comm, outdir / "demo_elasticity", "a") as outfile:
